...
static err_t low_level_init(struct netif *netif)
{
	...
	LWIP_DEBUGF(CORTEX_DEBUG, ("low_level_transmit: frame sent\n"));

	// set MAC hardware address
	ETHServiceTaskMACAddress(0, &(netif->hwaddr[0]));

	LWIP_DEBUGF(CORTEX_DEBUG, ("low_level_init: MAC address is \
%"X8_F"%"X8_F"%"X8_F"%"X8_F"%"X8_F"%"X8_F"\n",
	...
	LWIP_DEBUGF(CORTEX_DEBUG, ("low_level_init: MTU set to %"U16_F"\n",netif->mtu));
	...
	{
		ETHServiceTaskEnable(0);LWIP_DEBUGF(CORTEX_DEBUG, ("low_level_input: Waiting for Ethernet to become ready\n"));
...
static err_t low_level_transmit(struct netif *netif, struct pbuf *p)
{
	...
	if (0 == ETHServiceTaskLinkStatus(0))
	{
		...
		netif->flags &= ~NETIF_FLAG_LINK_UP;LWIP_DEBUGF(CORTEX_DEBUG, ("low_level_transmit: link is down\n"));
...
stellarisif_debug_print(struct pbuf *p)
{
	...
	LWIP_DEBUGF(CORTEX_DEBUG, ("ETH header:\n"));
	LWIP_DEBUGF(CORTEX_DEBUG, ("Packet Length:%5"U16_F" \n",*plen));
	LWIP_DEBUGF(CORTEX_DEBUG, ("Destination: %02"X8_F"-%02"X8_F"-%02"X8_F"-%02"X8_F"-%02"X8_F"-%02"X8_F"\n",
					ethhdr->dest.addr[0],
					ethhdr->dest.addr[1],
					ethhdr->dest.addr[2],
					ethhdr->dest.addr[3],
					ethhdr->dest.addr[4],
					ethhdr->dest.addr[5]));
	LWIP_DEBUGF(CORTEX_DEBUG, ("Source: %02"X8_F"-%02"X8_F"-%02"X8_F"-%02"X8_F"-%02"X8_F"-%02"X8_F"\n",
					ethhdr->src.addr[0],
					ethhdr->src.addr[1],
					ethhdr->src.addr[2],
					ethhdr->src.addr[3],
