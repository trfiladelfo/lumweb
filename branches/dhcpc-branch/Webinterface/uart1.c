...
//
//*****************************************************************************
#define TX_BUFFER_USED          (GetBuffer1Count(&g_ulUARTTxReadIndex,  \
                                                &g_ulUARTTxWriteIndex, \
                                                UART_TX_BUFFER_SIZE))
#define TX_BUFFER_FREE          (UART_TX_BUFFER_SIZE - TX_BUFFER_USED)
#define TX_BUFFER_EMPTY         (IsBuffer1Empty(&g_ulUARTTxReadIndex,   \
                                               &g_ulUARTTxWriteIndex))
#define TX_BUFFER_FULL          (IsBuffer1Full(&g_ulUARTTxReadIndex,  \
                                              &g_ulUARTTxWriteIndex, \
                                              UART_TX_BUFFER_SIZE))
...
//
//*****************************************************************************
#define RX_BUFFER_USED          (GetBuffer1Count(&g_ulUARTRxReadIndex,  \
                                                &g_ulUARTRxWriteIndex, \
                                                UART_RX_BUFFER_SIZE))
#define RX_BUFFER_FREE          (UART_RX_BUFFER_SIZE - RX_BUFFER_USED)
#define RX_BUFFER_EMPTY         (IsBuffer1Empty(&g_ulUARTRxReadIndex,   \
                                               &g_ulUARTRxWriteIndex))
#define RX_BUFFER_FULL          (IsBuffer1Full(&g_ulUARTRxReadIndex,  \
                                              &g_ulUARTRxWriteIndex, \
                                              UART_RX_BUFFER_SIZE))
...
#ifdef UART_BUFFERED
static tBoolean
IsBuffer1Full(volatile unsigned long *pulRead,
...
#ifdef UART_BUFFERED
static tBoolean
IsBuffer1Empty(volatile unsigned long *pulRead,
...
GetBuffer1Count(volatile unsigned long *pulRead,
               volatile unsigned long *pulWrite, unsigned long ulSize)
{
    unsigned long ulWrite;
    unsigned long ulRead;
...
#ifdef UART_BUFFERED
static void
UART1PrimeTransmit(unsigned long ulBase)
{
    ...
    if(!TX_BUFFER_EMPTY)
    {
        ...
        {
            MAP_UARTCharPutNonBlocking(ulBase, g_pcUARTTxBuffer[g_ulUARTTxReadIndex]);
...
//*****************************************************************************
void
UART1StdioInit(unsigned long ulPortNum)
{
    //
    // Check the arguments.
    //
...
//*****************************************************************************
int
UART1write(const char *pcBuf, unsigned long ulLen)
{
#ifdef UART_BUFFERED
    unsigned int uIdx;
...
//*****************************************************************************
int
UART1gets(char *pcBuf, unsigned long ulLen)
{
#ifdef UART_BUFFERED
...
//*****************************************************************************
void
UART1printf(const char *pcString, ...)
{
...
#if defined(UART_BUFFERED) || defined(DOXYGEN)
int
UART1Peek(unsigned char ucChar)
{
    int iCount;
    int iAvail;
...
#if defined(UART_BUFFERED) || defined(DOXYGEN)
void
UART1FlushTx(tBoolean bDiscard)
{
    unsigned long ulInt;
...
//! however, echo may be undesireable and this function can be used to disable
//! it.
//!
//! \return None.
//
