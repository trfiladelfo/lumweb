//*****************************************************************************
//
// sound.c - Sound driver for the Tempest EAC board.
//
...
SoundIntHandler(void)
{
    ...
    //
    // Get the DMA channel status, to see which channel is interrupting
    //
    ulStatus = HWREG(UDMA_CHIS);

    //
    // Clear the pending interrupts, but only the channels we are
    // servicing here.
    //
    HWREG(UDMA_CHIS) = (ulStatus & (1 << UDMA_CHANNEL_I2S0TX));

    //
    // Handle the RX channel interrupt
    //
    if(ulStatus & (1 << UDMA_CHANNEL_I2S0RX))
    {
        //
        // If the TX DMA is done, then start another one using the same
        // TX buffer.  This keeps the TX running continuously.
        //
           ...
           UDMA_MODE_STOP)
        {
            ...
            {
                g_sInBuffers[0].pfnBufferCallback(
                    g_sInBuffers[0].pulData, BUFFER_EVENT_FULL);
            }
        }
                ...
                UDMA_MODE_STOP)
        {
            ...
            {
                g_sInBuffers[1].pfnBufferCallback(
                    g_sInBuffers[1].pulData, BUFFER_EVENT_FULL);
            }
        }
    }
    ...
    //
    if(ulStatus & (1 << UDMA_CHANNEL_I2S0TX))
    {
        //
           ...
           UDMA_MODE_STOP)
        {
            ...
            {
                g_sOutBuffers[0].pfnBufferCallback(
                    g_sOutBuffers[0].pulData, BUFFER_EVENT_FREE);
                g_sOutBuffers[0].pulData = 0;
            }
        }
           ...
           UDMA_MODE_STOP)
        {
            ...
            {
                g_sOutBuffers[1].pfnBufferCallback(
                    g_sOutBuffers[1].pulData, BUFFER_EVENT_FREE);
                g_sOutBuffers[1].pulData = 0;
            }
        }
    }
