...
//
//*****************************************************************************
xSemaphoreHandle ETHRxBinSemaphore[MAX_ETH_PORTS] =
//pf { [0 ... (MAX_ETH_PORTS - 1)] = NULL };
		{ NULL };
...
//
//*****************************************************************************
xSemaphoreHandle ETHTxBinSemaphore[MAX_ETH_PORTS] =
//pf { [0 ... (MAX_ETH_PORTS - 1)] = NULL };
		{ NULL };
...
//
//*****************************************************************************
xSemaphoreHandle ETHTxAccessMutex[MAX_ETH_PORTS] =
//pf { [0 ... (MAX_ETH_PORTS - 1)] = NULL };
		{ NULL };
...
//
//*****************************************************************************
xSemaphoreHandle ETHRxAccessMutex[MAX_ETH_PORTS] =
//pf { [0 ... (MAX_ETH_PORTS - 1)] = NULL };
		{ NULL };
...
int ETHServiceTaskInit(const unsigned long ulPort)
{
	...
	if (ulPort < MAX_ETH_PORTS)
	{
		...
		GPIOPadConfigSet(ETHPortBase[ulPort], ETHPins[ulPort],
...
int ETHServiceTaskEnable(unsigned long ulPort)
{
	...
	if (ulPort < MAX_ETH_PORTS)
	{
		 ...
		 */
		EthernetConfigSet(ETHBase[ulPort], (ETH_CFG_TX_DPLXEN
		...
		//ma EthernetPHYWrite(ETHBase[ulPort], PHY_MR17, ETH_PHY_INT_MASK);
		...
		EthernetIntEnable(ETHBase[ulPort], ETH_INT_RX | ETH_INT_PHY
				| ETH_INT_RXOF | ETH_INT_TXER);
...
int ETHServiceTaskDisable(const unsigned long ulPort)
{
			...
			&& (HWREGBITW(&ETHDevice[ulPort], ETH_ENABLED)))
	{
		...
		// Disable the ETH transmit and receive interrupts.
		EthernetIntDisable(ETHBase[ulPort], (ETH_INT_RX | ETH_INT_PHY
				| ETH_INT_RXOF));
