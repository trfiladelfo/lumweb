...
JPEGDecompressImage(tJPEGWidget *pJPEG)
{
    ...
    pJPEG->psJPEGInst->pusImage = (unsigned short *)SDRAMAlloc(ulImageSize);
    ...
    pScanRows[0] = (JSAMPROW)SDRAMAlloc(cinfo.output_width *
                                        ulScanBufferHeight *
                                        cinfo.output_components);
    if(!pScanRows[0])
    {
        SDRAMFree(pJPEG->psJPEGInst->pusImage);
...
//! widget and returns the memory it was occupying to the SDRAM heap.  After
