          ...
          unsigned long ulRefresh)
{
    ...
    // Set the address map.
    //
    EPIAddressMapSet(EPI0_BASE, EPI_ADDR_RAM_SIZE_512MB | EPI_ADDR_RAM_BASE_6);
...
//*****************************************************************************
void *
SDRAMAlloc(unsigned long ulSize)
{
    if(g_bSDRAMPresent)
    {
        return(bget(ulSize));
...
//! using a call to SDRAMAlloc();
//!
//! \return None.
//
//*****************************************************************************
void
SDRAMFree(void *pvBlock)
{
    if(g_bSDRAMPresent)
    {
        brel(pvBlock);
...
//*****************************************************************************
unsigned long
SDRAMMaxFree(unsigned long *pulTotalFree)
{
