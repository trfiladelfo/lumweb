...
//
//*****************************************************************************
#define TX_BUFFER_USED          (GetBuffer0Count(&g_ulUARTTxReadIndex,  \
                                                &g_ulUARTTxWriteIndex, \
                                                UART_TX_BUFFER_SIZE))
#define TX_BUFFER_FREE          (UART_TX_BUFFER_SIZE - TX_BUFFER_USED)
#define TX_BUFFER_EMPTY         (IsBuffer0Empty(&g_ulUARTTxReadIndex,   \
                                               &g_ulUARTTxWriteIndex))
#define TX_BUFFER_FULL          (IsBuffer0Full(&g_ulUARTTxReadIndex,  \
                                              &g_ulUARTTxWriteIndex, \
                                              UART_TX_BUFFER_SIZE))
...
//
//*****************************************************************************
#define RX_BUFFER_USED          (GetBuffer0Count(&g_ulUARTRxReadIndex,  \
                                                &g_ulUARTRxWriteIndex, \
                                                UART_RX_BUFFER_SIZE))
#define RX_BUFFER_FREE          (UART_RX_BUFFER_SIZE - RX_BUFFER_USED)
#define RX_BUFFER_EMPTY         (IsBuffer0Empty(&g_ulUARTRxReadIndex,   \
                                               &g_ulUARTRxWriteIndex))
#define RX_BUFFER_FULL          (IsBuffer0Full(&g_ulUARTRxReadIndex,  \
                                              &g_ulUARTRxWriteIndex, \
                                              UART_RX_BUFFER_SIZE))
...
#ifdef UART_BUFFERED
static tBoolean
IsBuffer0Full(volatile unsigned long *pulRead,
...
#ifdef UART_BUFFERED
static tBoolean
IsBuffer0Empty(volatile unsigned long *pulRead,
...
GetBuffer0Count(volatile unsigned long *pulRead,
               volatile unsigned long *pulWrite, unsigned long ulSize)
{
    unsigned long ulWrite;
    unsigned long ulRead;
...
#ifdef UART_BUFFERED
static void
UART0PrimeTransmit(unsigned long ulBase)
{
...
//*****************************************************************************
void
UART0StdioInit(unsigned long ulPortNum)
{
    //
    // Check the arguments.
    //
...
//*****************************************************************************
int
UART0write(const char *pcBuf, unsigned long ulLen)
{
#ifdef UART_BUFFERED
    unsigned int uIdx;
...
//*****************************************************************************
int
UART0gets(char *pcBuf, unsigned long ulLen)
{
#ifdef UART_BUFFERED
...
//*****************************************************************************
void
UART0printf(const char *pcString, ...)
{
...
#if defined(UART_BUFFERED) || defined(DOXYGEN)
int
UART0Peek(unsigned char ucChar)
{
    int iCount;
    int iAvail;
...
#if defined(UART_BUFFERED) || defined(DOXYGEN)
void
UART0FlushTx(tBoolean bDiscard)
{
    unsigned long ulInt;
