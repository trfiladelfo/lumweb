...
//
//*****************************************************************************
static unsigned long g_ulUartBase[3] = { UART0_BASE, UART1_BASE, UART2_BASE };
...
//*****************************************************************************
#ifdef UART_BUFFERED
static unsigned long g_ulUartInt[3] = { INT_UART0, INT_UART1, INT_UART2 };
#endif
...
//
//*****************************************************************************
static unsigned long g_ulUartPeriph[3] = { SYSCTL_PERIPH_UART0,
...
//*****************************************************************************
#ifdef UART_BUFFERED
static tBoolean IsBufferFull(unsigned long volatile *pulRead,
...
//*****************************************************************************
#ifdef UART_BUFFERED
static tBoolean IsBufferEmpty(unsigned long volatile *pulRead,
...
//*****************************************************************************
#ifdef UART_BUFFERED
static unsigned long GetBufferCount(unsigned long volatile *pulRead,
		unsigned long volatile *pulWrite, unsigned long ulSize) {
	unsigned long ulWrite;
	unsigned long ulRead;
	...
	return ((ulWrite >= ulRead) ? (ulWrite - ulRead) : (ulSize - (ulRead
			- ulWrite)));
}
#endif
...
//*****************************************************************************
#ifdef UART_BUFFERED
static void UARTPrimeTransmit(unsigned long ulBase) {
	//
...
//
//*****************************************************************************
static int UARTwrite(const char *pcBuf, unsigned long ulLen) {
#ifdef UART_BUFFERED
	unsigned int uIdx;
	...
	//
	ASSERT(pcBuf != 0);ASSERT(g_ulBase != 0);
...
int UARTgets(char *pcBuf, unsigned long ulLen) {
	...
	// Check the arguments.
	//
	ASSERT(pcBuf != 0);ASSERT(ulLen != 0);ASSERT(g_ulBase != 0);
...
void UARTStdioIntHandler(void) {
	...
	if (ulInts & (UART_INT_RX | UART_INT_RT)) {
		RIT128x96x4StringDraw("Empfangen", 10, 30, 0xf);
		//
		...
		while (MAP_UARTCharsAvail(g_ulBase)) {
			...
			if (!RX_BUFFER_FULL) {
				...
				//
				g_pcUartRxBuffer[g_ulUartRxWriteIndex] = (unsigned char) (lChar
						& 0xFF);
